...
// Updater or Setter owns the objects it writes, so it creates them when absent and deletes them on
...
// consumeDeltas folds the event into the pending set and flushes it: the
// shared Consume implementation for the delta-typed consumers (Patcher and
// Updater).
func (c *baseConsumer) consumeDeltas(ctx context.Context, out dbspruntime.Event) error {
	c.writeM.Lock()
